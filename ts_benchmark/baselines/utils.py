...
class DatasetForTransformer:
    ...
    def __getitem__(self, index):
        ...
        if self.pre is not 'bf16':
            ...
            seq_x = torch.tensor(seq_x.values, dtype=torch.bfloat16)
            seq_y = torch.tensor(seq_y.values, dtype=torch.bfloat16)
            seq_x_mark = torch.tensor(seq_x_mark, dtype=torch.bfloat16)
            seq_y_mark = torch.tensor(seq_y_mark, dtype=torch.bfloat16)
